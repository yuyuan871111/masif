...
def compute_thetas(plane, vix, verts, faces, normal, neighbors, idx):
    ...
    angle_v1_v2 = np.arctan2(norm(np.cross(v2, v1)), np.dot(v2, v1)) * np.sign(
        np.dot(v2, np.cross(normal_tt, v1))
    )
...
def compute_theta_all(D, vertices, faces, normals, idx, radius):
    mymds = MDS(
        n_components=2, n_init=1, max_iter=50, dissimilarity="precomputed", n_jobs=10
    )
    all_theta = []
