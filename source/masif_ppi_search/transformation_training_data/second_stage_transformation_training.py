...
for target_ix, target_pdb in enumerate(rand_list):
    ...
    for source_ix, source_pdb in enumerate(rand_list):
        source_desc = np.load(
            os.path.join(desc_dir, source_pdb, "p2_desc_straight.npy")
        )
    ...
    # Assume 9A patches.
    target_coord = np.load(
        os.path.join(precomp_dir_9A, target_pdb, "p1_list_indices.npy")
    )
    ...
    for source_ix, source_pdb in enumerate(rand_list):
        ...
        source_pcd = read_point_cloud(
            os.path.join(surf_dir, "{}.ply".format(pdb_id + "_" + chain))
        )

        source_desc = np.load(
            os.path.join(desc_dir, source_pdb, "p2_desc_straight.npy")
        )
        source_coords = np.load(
            os.path.join(precomp_dir_9A, source_pdb, "p2_list_indices.npy")
        )
        ...
        all_results, all_source_patch, all_source_descs = multidock(
            source_pcd,
            source_coords,
            source_desc,
            source_vix,
    ...
    np.save(
        os.path.join(outdir, "aligned_source_patches"),
        np.asarray(aligned_source_patches),
    )
    np.save(
        os.path.join(outdir, "aligned_source_normals"),
        np.asarray(aligned_source_normals),
    )
    np.save(
        os.path.join(outdir, "aligned_source_patches_descs"),
        np.asarray(aligned_source_patches_descs),
    )
    ...
    np.save(
        os.path.join(outdir, "target_patch_normals"), np.asarray(target_patch.normals)
    )
    np.save(
        os.path.join(outdir, "target_patch_descs"),
        np.asarray(target_patch_descs.data).T,
    )
